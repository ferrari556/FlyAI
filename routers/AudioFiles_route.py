...
async def create_audiofile(audiofile: audiofiles, db: AsyncSession = Depends(get_db)):
    try:
        ...
        await db.commit()
        await db.refresh(new_audiofile)
...
async def update_audiofile(audio_id: int, audiofile: audiofiles, db: AsyncSession = Depends(get_db)):
    try:
        ...
        await db.commit()
        await db.refresh(existing_audiofile)
