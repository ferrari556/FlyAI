...
@router.get("/bgm/read")
async def get_all_bgm_sounds(db: AsyncSession = Depends(get_db)):
    async with db as session:
        query = select(BackgroundMusic)  # EffectSounds 모델의 모든 데이터를 조회하는 쿼리
        result = await session.execute(query)
        ...
        bgm_data = [
            {
                "bgm_id": bgm.bgm_id,
                "audio_id": bgm.audio_id,
                "Music_Name": bgm.Music_Name,
                "MusicFilePath": bgm.MusicFilePath,
                "File_Length": bgm.File_Length,
                "Upload_Date": bgm.Upload_Date.isoformat(),
            }
            for bgm in bgms
        ]

        # 변환된 데이터를 JSONResponse 객체로 반환
        return JSONResponse(status_code=200, content={"bgms": bgm_data})
